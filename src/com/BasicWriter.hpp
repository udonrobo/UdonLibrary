...
class BasicWriter {
	...
	public:
		...
		/// @param value 配列
		inline constexpr void setArrayData(uint8_t (&value)[Size]) {
		...
		/// @param index バイト番地
		/// @param value 値 (0 ~ 255)
		inline constexpr void setByteData(const size_t index, const uint8_t value) {
			buffer[index] = value;
		}
		...
		inline constexpr void setBitData(const size_t byteIndex, const size_t bitIndex, const bool value) {
		...
		template<class Ty>
		inline constexpr void setMessage(const Ty& value) {
