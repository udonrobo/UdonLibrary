 ...
 * to 1 byte
 */
inline Adafruit_BusIO_Register::Adafruit_BusIO_Register(Adafruit_I2CDevice *i2cdevice,
                                                 uint16_t reg_addr,
                                                 uint8_t width,
                                                 uint8_t byteorder,
 ...
 * to 1 byte
 */
inline Adafruit_BusIO_Register::Adafruit_BusIO_Register(Adafruit_SPIDevice *spidevice,
                                                 uint16_t reg_addr,
 ...
 * to 1 byte
 */
inline Adafruit_BusIO_Register::Adafruit_BusIO_Register(
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_Register::write(uint8_t *buffer, uint8_t len) {
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_Register::write(uint32_t value, uint8_t numbytes) {
  if (numbytes == 0) {
    numbytes = _width;
  }
  if (numbytes > 4) {
 ...
 */
inline uint32_t Adafruit_BusIO_Register::read(void) {
 ...
 *    @return Returns 0xFFFFFFFF on failure, value otherwise
 */
inline uint32_t Adafruit_BusIO_Register::readCached(void) { return _cached; }
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_Register::read(uint8_t *buffer, uint8_t len) {
  uint8_t addrbuffer[2] = {(uint8_t)(_address & 0xFF),
                           (uint8_t)(_address >> 8)};
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_Register::read(uint16_t *value) {
  if (!read(_buffer, 2)) {
    return false;
  }
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_Register::read(uint8_t *value) {
  if (!read(_buffer, 1)) {
    return false;
  }
 ...
 */
inline void Adafruit_BusIO_Register::print(Stream *s) {
  uint32_t val = read();
  s->print("0x");
  s->print(val, HEX);
}
 ...
 *    @param  s The Stream to print to, defaults to &Serial
 */
inline void Adafruit_BusIO_Register::println(Stream *s) {
  print(s);
  s->println();
}
 ...
 */
inline Adafruit_BusIO_RegisterBits::Adafruit_BusIO_RegisterBits(
 ...
 */
inline uint32_t Adafruit_BusIO_RegisterBits::read(void) {
 ...
 * uncheckable)
 */
inline bool Adafruit_BusIO_RegisterBits::write(uint32_t data) {
  uint32_t val = _register->read();
 ...
 */
inline uint8_t Adafruit_BusIO_Register::width(void) { return _width; }

/*!
 *    @brief  Set the default width of data
 *    @param width the default width of data read from register
 */
inline void Adafruit_BusIO_Register::setWidth(uint8_t width) { _width = width; }

/*!
 *    @brief  Set register address
 *    @param address the address from register
 */
inline void Adafruit_BusIO_Register::setAddress(uint16_t address) {
  _address = address;
}
 ...
 */
inline void Adafruit_BusIO_Register::setAddressWidth(uint16_t address_width) {
  _addrwidth = address_width;
}
