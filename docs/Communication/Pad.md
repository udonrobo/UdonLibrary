...
コントローラーの情報は複数のデバイスを介して取得されます。デバイスの構成にはいくつか組み合わせがあり、使用環境に応じて適切な組み合わせを選択する必要があります。以下にいくつかの組み合わせパターンを示します。

本部はロボコンで基本的に 920MHz 帯を使用します。ゲーム機としてのコントローラは Bluetooth を用いて通信を行いますが、ロボコン会場には WiFi や Bluetooth が多く存在し混線する可能性があります。そこで 920MHz 帯を用いてロボットへ送信します。

コントローラーと送信基板は USB を用いて有線で接続します。

### 920MHz 経由 / 無線モジュールがメイン基板に含まれる場合
...
### 920MHz 経由 / 無線モジュールがメイン基板と分離している場合
...
試作ロボットの動作、混線が起こりにくい環境下では Bluetooth を使用します。

### Bluetooth 経由 / USB ホストシールドがメイン基板に含まれる場合
...
### Bluetooth 経由 / USB ホストシールドがメイン基板と分離している場合
